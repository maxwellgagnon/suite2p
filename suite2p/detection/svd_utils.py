...
def vmap_from_usv(us, ss, vs, rechunk_for_var = True, intensity_thresh=None):
    ...
    for zidx in range(nz):
        vmap_i = svd_variance(us[:,zidx], ss[:,zidx], vs_flat[:,zidx], rechunk_comps=rechunk_for_var).reshape(nyb,nxb)
        vmaps.append(vmap_i)
...
def filter_v(v, filter_shape, mode='constant', norm=True):
    if norm:
        filt_mask = dask_uniform_filter(darr.ones_like(v), filter_shape, mode=mode)
...
def reconstruct_from_stack(us, ss, vs, rechunk_comps='max', z_chunks=4, n_comp_reconstruct=None, dtype=None):
    nz, n_comp = ss.shape
    nt = us.shape[0]
    reshape = False
    if len(vs.shape) > 3:
        reshape = True
        img_shape = vs.shape[2:]
        flat_shape = n.product(img_shape)
        vs = vs.reshape(*vs.shape[:2], flat_shape)
    ...
    for i in range(nz):
        ...
        mov3d.append(mov_i)
    mov3d = darr.stack(mov3d, axis=1).rechunk((None, z_chunks, None))

    if reshape:
        mov3d = mov3d.reshape(nt, nz, *img_shape, limit='10GiB')
    return mov3d
...
def block_and_svd(mov_reg, n_comp, block_shape = (1,128,128), block_overlaps=(0, 18, 18), 
    ...
    log_cb("Will compute SVD in %d blocks in a grid shaped %s"% (n_blocks, str(grid_shape)), 1)

    n_batches = n.ceil(n_blocks / n_svd_blocks_per_batch)
    log_cb("Batching %d blocks together, for a total of %d batches" % (n_svd_blocks_per_batch, n_batches))
    ...
    for batch_start in range(0, n_blocks, n_svd_blocks_per_batch):
        ...
        log_cb("Starting batch %d / %d, blocks %d - %d" % (batch_idx, n_batches, batch_start, batch_end),2)
        to_compute = []
        ...
        est_time_str = time.strftime("%Hh%Mm%Ss", time.gmtime(est_remaining_time))
